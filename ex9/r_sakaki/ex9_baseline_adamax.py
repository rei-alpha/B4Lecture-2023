...
def plot_confusion_matrix(predict, ground_truth, title=None, cmap=plt.cm.Blues):
    ...
    plt.tight_layout()
    plt.savefig("result/cm_baseline_adamax100.png", transparent=True)
    plt.show()
...
def write_result(paths, outputs):
    ...
    with open("result_baseline_adamax100.csv", "w") as f:
        f.write("path,output\n")
...
def plot_history(history):
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    plt.savefig("result/history_baseline_adamax100_acc.png", transparent=True)
    plt.show()
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.savefig("result/history_baseline_adamax100_loss.png", transparent=True)
    plt.show()
...
def main():
    ...
    # モデルの学習基準の設定
    model.compile(
        loss="categorical_crossentropy", optimizer=Adamax(lr=1e-4), metrics=["accuracy"]
    )
    ...
    # モデル構成，学習した重みの保存
    model.save("keras_model/my_model_baseline_adamax100.h5")
